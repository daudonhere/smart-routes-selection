...
export default function RouteResultCard({ route, totalCost, currencySymbol }: RouteResultCardProps) {
  ...
  return (
    ...
    >
      <p className="flex items-center gap-1.5 font-bold color-quinary text-sm 2xl:text-lg">
        <Info size={14} className={`block ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:hidden`} />
      ...
      <div className="flex flex-col w-full mt-2.5 gap-1 lg:gap-1.5">
        <div className="flex flex-row gap-1.5 lg:gap-2.5">
          <div className='flex flex-row w-full items-center gap-1'>
            <Route size={14} className={`block ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:hidden`} />
            <Route size={20} className={`hidden ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:block`} />
            <span className="font-bold color-quinary text-xs 2xl:text:base">{route.distance.toFixed(2)} Km</span>
          </div>
          <div className='flex flex-row w-full items-center gap-1'>
            <Gauge size={14} className={`block ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:hidden`} />
            <Gauge size={20} className={`hidden ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:block`} />
            <span className="font-bold color-quinary text-xs 2xl:text:base">
              {`${lowerSpeed.toFixed(0)} - ${upperSpeed.toFixed(0)} Kmh`}
            </span>
          </div>
        </div>
        <div className="flex flex-row gap-1 lg:gap-2.5">
          <div className='flex flex-row w-full items-center gap-1'>
            <Timer size={14} className={`block ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:hidden`} />
            <Timer size={20} className={`hidden ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:block`} />
            <span className="font-bold color-quinary text-xs 2xl:text:base">{route.duration.toFixed(0)} Minutes</span>
          </div>
          <div className='flex flex-row w-full items-center gap-1'>
            <TrafficCone size={14} className={`block ${route.isPrimary ? 'color-senary' : 'color-quinary'} 2xl:hidden`} />
        ...
        <div className="flex flex-col mt-1 gap-1">
          {isSlowSpeed && (
            <div className="flex items-center gap-2 font-semibold color-senary text-xs 2xl:text:base">
              <AlertTriangle size={14} className="block 2xl:hidden" />
          ...
          {showTollFeeWarning && (
              <div className="flex items-center gap-2 font-semibold color-senary text-xs 2xl:text:base">
                <CircleDollarSign size={14} className="block 2xl:hidden" />
        ...
        <div className="flex flex-row mt-2 pt-1.5 border-t line-senary gap-2">
          <div className="flex flex-1 w-full items-center">
            <p className="flex items-center gap-2 font-medium color-senary">
              <span className={`font-bold color-quinary ${route.isPrimary ? 'text-sm 2xl:text-base' : 'text-xs 2xl:text-sm'}`}>
          ...
          <div className="flex flex-1 w-full">
            <button
              ...
              className={`w-full py-2 font-bold rounded-sm transition-colors focus:outline-none focus:shadow-outline text-xs 2xl:text:sm ${route.isPrimary ? 'background-senary hover:bg-yellow-300 color-primary cursor-pointer' : 'background-quaternary color-quinary'} disabled:cursor-not-allowed disabled:opacity-80`}
            >
              {buttonText}
            </button>
          </div>
