    ...
    >
      ...
      <div className="flex flex-col gap-2 w-full mt-1">
        ...
        <div className="flex flex-col gap-1 mt-1">
          {isSlowSpeed && (
            <div className="flex items-center gap-2 color-nonary text-xs"><AlertTriangle size={16} /><span>Route maybe congested</span></div>
          )}
          {showTollFeeWarning && (
              <div className="flex items-center gap-2 color-nonary text-xs"><CircleDollarSign size={16} /><span>Price exclude toll fees</span></div>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-row gap-2 mt-3 pt-2 border-t line-quinary">
          ...
          <div className="flex flex-1 w-full">
            <button
              ...
              className={`w-full font-bold text-xs rounded-sm focus:outline-none focus:shadow-outline transition-colors py-2 ${route.isPrimary ? 'background-tertiary hover:bg-yellow-400 color-primary cursor-pointer' : 'background-quaternary color-doctary'} disabled:cursor-not-allowed disabled:opacity-60`}
            >
              {buttonText}
            </button>
          </div>
